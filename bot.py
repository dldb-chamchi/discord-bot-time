...
from config import REPORT_CHANNEL_ID_DEPLOY
...
async def on_ready():
    ...
    # 배포 완료 알림 (커밋 정보 포함)
    # ---------------------------------------------------------
    if REPORT_CHANNEL_ID_DEPLOY:
        try:
            channel = bot.get_channel(REPORT_CHANNEL_ID_DEPLOY)
            if not channel:
                channel = await bot.fetch_channel(REPORT_CHANNEL_ID_DEPLOY)
